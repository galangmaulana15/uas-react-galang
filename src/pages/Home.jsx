  ...
  const Home = () => {
      ...
      const [favorites, setFavorites] = useState(() => {
          const saved = localStorage.getItem('roncomovie_favorites');
          return saved ? JSON.parse(saved) : [];
      });

      const carouselRef = useRef(null);
      const totalSlides = 4; // 4 slide untuk 12 film (3 film per slide)
      ...
      useEffect(() => {
          const fetchMovies = async () => {
              try {
                  ...
                  // Ambil lebih banyak data untuk popular movies
                  setPopularMovies(popularData.movies.slice(0, 16)); // 16 film untuk carousel dan grid
                  setTopRatedMovies(topRatedData.movies.slice(0, 8)); // 8 film
                  setUpcomingMovies(upcomingData.movies.slice(1,5)); 
                  setNowPlayingMovies(nowPlayingData.movies.slice(0, 8)); // 8 film untuk Now Playing
              } catch (error) {
